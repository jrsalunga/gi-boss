...
class CshAudtController extends Controller { 
	...
	private function getFileSystem($id, $action, $p) { 
		...
		$paths = [];
		$r = $this->files->folderInfo('CSHAUDT');
      ...
      arsort($dirs);
			...
			$root = $this->files->folderInfo('CSHAUDT/'.$action.'/'.strtoupper($id));
      arsort($root['subfolders']);
			$data = [
				'folder' 			=> "/CSHAUDT/".strtoupper($id).'/'.$action,
				'folderName' 	=> $action,
				'breadcrumbs' => [
					'/' 				=> "Storage",
					'/CSHAUDT'		=> "CSHAUDT",
					'/CSHAUDT/'.strtoupper($id) 	=> strtoupper($id),
				],
			...
			$root = $this->files->folderInfo('CSHAUDT/'.$action.'/'.strtoupper($id).'/'.$p);
      arsort($root['files']);
			$data = [
				'folder' 			=> "/CSHAUDT/".strtoupper($id).'/'.$action.'/'.$p,
				'folderName' 	=> $p,
				'breadcrumbs' => [
					'/' 				=> "Storage",
					'/CSHAUDT'		=> "CSHAUDT",
					'/CSHAUDT/'.strtoupper($id) 	=> strtoupper($id),
					'/CSHAUDT/'.strtoupper($id).'/'.$action 	=> $action,
				],
				'subfolders'	=> $root['subfolders'],
				'files'				=> $root['files']
			];
		...
		return view('docu.cshaudt.filelist')->with('data', $data);
	}
	
	private function getPath($d) {
		return 'CSHAUDT'.DS.$d->date->format('Y').DS.strtoupper($d->branch->code).DS.$d->date->format('m').DS.$d->filename;
	}
