...
class EmpController extends Controller
{
	...
	private function toDbfAttributes(Employee $employee, $cmd) {
		...
		return [
			...
			'LAST_NAM'		=> eyne(up(substr($employee->lastname, 0, 20))), //['LAST_NAM', 	'C', 20],	
			'FIRS_NAM'		=> eyne(up(substr($employee->firstname, 0, 20))),	//['FIRS_NAM', 	'C', 20],	
			'MI'					=> up(substr($employee->middlename, 0, 1)),	//['MI', 				'C', 2],	
			'MIDL_NAM'		=> eyne(up(substr($employee->middlename, 0, 20))), //['MIDL_NAM', 	'C', 20],	
			...
			'CHILDREN1'		=> eyne(up($c[0])), //['CHILDREN1', 'C', 25],	
			'CHILDREN2'		=> eyne(up($c[1])), //['CHILDREN2', 'C', 25],	
