...
class ExpenseController extends Controller
{
  ...
  private $mSaleType;
    ...
    if (!is_null($branch)) {
    	...
    	$ms = $this->ms->skipCache()->findWhere(['date'=>$this->dr->to->format('Y-m-d'), 'branch_id'=>$branch->id], ['date', 'slsmtd_totgrs', 'sales', 'food_sales', 'fc', 'transcos', 'cos', 'ave_sales', 'rank'])->first();
      ...
      if (!in_array($request->user()->id, ['41F0FB56DFA811E69815D19988DDBE1E', '11E943EA14DDA9E4EAAFBD26C5429A67'])) {
        ...
        \Mail::queue('emails.notifier', $email, function ($m) {
          ...
          $m->to('giligans.log@gmail.com')->subject('PNL');
        });
      }
  ...
  public function getMonthRangePnl(Request $request) {
    ...
    if (!in_array($request->user()->id, ['41F0FB56DFA811E69815D19988DDBE1E', '11E943EA14DDA9E4EAAFBD26C5429A67'])) {
      ...
      \Mail::queue('emails.notifier', $email, function ($m) {
        $m->from('giligans.app@gmail.com', 'GI App - Boss');
        $m->to('giligans.log@gmail.com')->subject('PNL Month Range');
      });
    }
