...
class KitlogRepository extends BaseRepository implements CacheableInterface
{
  ...
  public function aggregateFoodAllByDr(Carbon $fr, Carbon $to) {
  ...
  public function aggregateAllAreaDatasetByDr(Carbon $fr, Carbon $to) {
    ...
    return $this->scopeQuery(function($query) use ($fr, $to) {
      return $query
                ...
                ->select(DB::raw("LAST_DAY(date) as date, area, 1*floor(minute/1) as grp, count(minute) as txn, round(sum(qty),0) as qty"))
                ->whereBetween('date', 
                  [$fr->format('Y-m-d'), $to->format('Y-m-d')]
                  )
                ->where('qty', '>', 0)
  ...
  public function aggregateAllProductDatasetByDr(Carbon $fr, Carbon $to) {
    ...
    return $this->scopeQuery(function($query) use ($fr, $to) {
      return $query
                ...
                ->select(DB::raw("LAST_DAY(date) as date, product_id, 1*floor(minute/1) as grp, count(minute) as txn, round(sum(qty),0) as qty"))
                ->whereBetween('date', 
                  [$fr->format('Y-m-d'), $to->format('Y-m-d')]
                  )
                ->where('qty', '>', 0)
  ...
  public function rawAggregateDatasetByDr(Carbon $fr, Carbon $to) {

    return $this->scopeQuery(function($query) use ($fr, $to) {
      return $query
                ->select(DB::raw("LAST_DAY(date) as date, product_id, area, concat(2*floor(minute/2), '-', 2*floor(minute/2) + 1) as grp, count(minute) as txn, round(sum(qty),0) as qty"))
                ->whereBetween('date', 
                  [$fr->format('Y-m-d'), $to->format('Y-m-d')]
                  )
                ->where('qty', '>', 0);
