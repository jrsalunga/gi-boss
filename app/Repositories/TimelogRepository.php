...
{ 
  ...
  public function getActiveEmployees($branchid, $field = NULL) {
    ...
    return $this->employees->with('position')
    						//->skipCache()
    						->pushCriteria(new ActiveEmployee())
                ->orderBy('lastname')
                ->orderBy('firstname')
                ->findWhere(['branchid'=>$branchid], $field);
                /*
  ...
  public function allByDate(Carbon $date, $branchid) {

    $arr = [];
    $timelogs = [];
    // get all timelog on the day/date
    $raw_timelogs = $this->allTimelogByDate($date, $branchid)->all();
    //$raw_timelogs = ;
    $tk_empids =  $raw_timelogs->pluck('employeeid')->toArray();
    $employees = $this->getActiveEmployees($branchid);
