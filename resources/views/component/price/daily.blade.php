      ...
      <table class="table table-hover table-sort-all">
        <thead>
          <tr>
            ...
            <th class="text-right">Last Purch</th>
        ...
        <?php 
          if(in_array($data['component']->code, $bb->pluck('code')->toArray())) {
            $b = $bb->keyBy('code')->get($data['component']->code);
            $in = 'bg-success';
          } else {
            $in = false;
          }
          ...
          <tr class="{{ $in }}">
            <td>
              @if(!$in)
                {{ $data['component']->code }}
              @else
                <a href="/component/purchases?table=component&item={{$filter->item}}&itemid={{$filter->id}}&fr={{$dr->fr->format('Y-m-d')}}&to={{$dr->to->format('Y-m-d')}}&branchid={{$b->lid()}}">
                  <b>{{ $data['component']->code }}</b>
                </a>
              @endif
            </td>
