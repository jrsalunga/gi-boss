...
@section('title', '- Lessor Update')

@section('body-class', 'lessor-update')
	...
	<div class="col-md-12">
		<h3 class="page-header">{{ $lessor->descriptor }} <small>{{ $lessor->code }}</small>
			@if(request()->has('type') && request()->input('type')==='full')
				<a href="/masterfiles/lessor/{{$lessor->lid()}}/edit" class="pull-right" title="Toogle few"><i class="material-icons">more_horiz</i></a>
			@else
				<a href="/masterfiles/lessor/{{$lessor->lid()}}/edit?type=full" class="pull-right" title="Toogle full"><i class="material-icons">more_vert</i></a>
			@endif
		</h3>
		...
		<div class="row">
			<div class="col-md-4">
			    ...
			    <input type="text" class="form-control" id="code" name="code" placeholder="Lessor Code" maxlength="3" value="{{ $lessor->code }}">
			  </div>
			  <div class="form-group">
			    <label for="descriptor">Descriptor</label>
			    <input type="text" class="form-control" id="descriptor" name="descriptor" placeholder="Descriptor" maxlength="50" value="{{ $lessor->descriptor }}">
			  </div>
			  @endif
			  <div class="form-group">
			    <label for="trade_name">Trade Name</label>
			    <input type="text" class="form-control" id="trade_name" name="trade_name" placeholder="Trade Name" maxlength="50" value="{{ $lessor->trade_name }}">
			  </div>
			  <div class="form-group">
			    <label for="address">Address</label>
			    <textarea class="form-control" id="address" name="address" placeholder="Address" maxlength="120" style="max-width: 100%;min-width: 100%;" rows="5">{{ $lessor->address }}</textarea>
			  </div>
			  <div class="form-group">
			    <label for="email">Email</label>
			    <input type="text" class="form-control" id="email" name="email" placeholder="Email" maxlength="50" value="{{ $lessor->email }}">
			  </div>
			  <div class="form-group">
			    <label for="tin">TIN</label>
			    <input type="text" class="form-control" id="tin" name="tin" placeholder="000-000-000-000" maxlength="16" value="{{ $lessor->tin }}" data-mask="000-000-000-000">
			  </div>
			</div>
			<div class="col-md-4">
			  <div class="input_fields_wrap">
			    	...
			    	@else 
							@foreach($lessor->contacts as $k => $contact)
		...
		<div class="row">
			<div class="col-md-6">
			  ...
			  <input type="hidden" name="id" value="{{$lessor->lid()}}">
			  <button type="submit" class="btn btn-primary" data-toggle="loader"><span class="gly gly-floppy-saved"></span> Update</button>
			  <a href="/masterfiles/lessor/{{ $lessor->lid() }}" class="btn btn-default" data-toggle="loader"><span class="gly gly-remove"></span> Cancel</a>
			</div>
		</div><!-- end:.row -->
		</form>
...
@section('js-external')
  ...
  $(document).ready(function() {
    ...
    var max_fields      = <?=(count($lessor->contacts)<=0)?4:5-count($lessor->contacts)?>; //maximum input boxes allowed
