      ...
      <ul class="dropdown-menu hidden-md hidden-lg">
        @foreach($option['dropdown'] as $ddk => $ddv)
          ...
          @else
            <li>
          @endif
          <a href="/{{ $controller }}/{{ $ddk }}" data-toggle="loader">{{ $ddv['caption'] }}</a></li>
        @endforeach
      </ul>
      
      <a href="/{{ $controller }}" class="hidden-xs hidden-sm">{{ $option['caption'] }}</a>
    @else   
        <a href="/{{ $controller }}">{{ $option['caption'] }}</a>
    @endif
